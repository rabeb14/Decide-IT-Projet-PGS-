 ...
 * @status 
 */
public class dep implements TalendJob {
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	private final String jobName = "dep";
		...
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(dep.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
    final static byte[] commonByteArrayLock_PGS_STAGE_dep = new byte[0];
    static byte[] commonByteArray_PGS_STAGE_dep = new byte[0];
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_PGS_STAGE_dep) {
 ...
 */
	...
	final String decryptedPassword_tSendMail_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:BDQBpKDOXcOAA+28VM9oShFSAvLnwqVl+nOsmb7UQuyJhRpHt36mkuAuVkIZDbA=");
 ...
 */
    ...
    public static void main(String[] args){
        final dep depClass = new dep();

        int exitCode = depClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = dep.class.getClassLoader().getResourceAsStream("pgs_stage/dep_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = dep.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : dep");
        }
...
/************************************************************************************************
 *     48420 characters generated by Talend Open Studio for Data Integration 
