...
package pgs_stage.dim_api_copy_0_1;
...
/**
 * Job: DIM_API_Copy Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class DIM_API_Copy implements TalendJob {
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	private final String jobName = "DIM_API_Copy";
		...
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(DIM_API_Copy.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class out1Struct implements routines.system.IPersistableRow<out1Struct> {
    final static byte[] commonByteArrayLock_PGS_STAGE_DIM_API_Copy = new byte[0];
    static byte[] commonByteArray_PGS_STAGE_DIM_API_Copy = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_PGS_STAGE_DIM_API_Copy.length) {
				if(length < 1024 && commonByteArray_PGS_STAGE_DIM_API_Copy.length == 0) {
   					commonByteArray_PGS_STAGE_DIM_API_Copy = new byte[1024];
				} else {
   					commonByteArray_PGS_STAGE_DIM_API_Copy = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_PGS_STAGE_DIM_API_Copy, 0, length);
			strReturn = new String(commonByteArray_PGS_STAGE_DIM_API_Copy, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_PGS_STAGE_DIM_API_Copy, 0, length);
			strReturn = new String(commonByteArray_PGS_STAGE_DIM_API_Copy, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_PGS_STAGE_DIM_API_Copy) {
...
String dbUser_tDBOutput_1 = null;
	...
	final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:37t5PT2fneobZJtXhTt4/uQ8CMAZoUTjtDnMKSJFD3cMC0jTTTU=");
 ...
 */
	...
	final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:rlbddmF0jksCytota3pAPFhbgVG+zKa/6kJJUexZOufRH7QVIQs=");
...
} else {pstmt_tDBOutput_1.setString(2, out1.API_CODE);
}

                    if(out1.DESCRIPTION == null) {
pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(3, out1.DESCRIPTION);
}

                    pstmt_tDBOutput_1.setInt(4, out1.USER_DEFINED);

                    pstmt_tDBOutput_1.setInt(5, out1.DENSITY_UNITS);

                    pstmt_tDBOutput_1.setInt(6, out1.DENSITY_PRECISION);

                    pstmt_tDBOutput_1.setInt(7, out1.COE_PRECISION);

                    pstmt_tDBOutput_1.setInt(8, out1.WEIGHT_IN_AIR_METHOD);

                    pstmt_tDBOutput_1.setInt(9, out1.TEMPERATURE_UNITS);

                    pstmt_tDBOutput_1.setDouble(10, out1.REFERENCE_TEMPERATURE);

                    pstmt_tDBOutput_1.setInt(11, out1.TEMPERATURE_PRECISION);

                    pstmt_tDBOutput_1.setDouble(12, out1.MAXIMUM_PRESSURE);

                    pstmt_tDBOutput_1.setInt(13, out1.COMMODITY_GROUP);

                    if(out1.DLL == null) {
pstmt_tDBOutput_1.setNull(14, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(14, out1.DLL);
}

                    pstmt_tDBOutput_1.setInt(15, out1.VOLUME_CORRECTION_METHOD);

                    if(out1.VOLUME_CORRECTION_ROUTINE == null) {
pstmt_tDBOutput_1.setNull(16, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(16, out1.VOLUME_CORRECTION_ROUTINE);
}

                    pstmt_tDBOutput_1.setInt(17, out1.DENSITY_CORRECTION_METHOD);
 ...
 */
    ...
    public static void main(String[] args){
        final DIM_API_Copy DIM_API_CopyClass = new DIM_API_Copy();

        int exitCode = DIM_API_CopyClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = DIM_API_Copy.class.getClassLoader().getResourceAsStream("pgs_stage/dim_api_copy_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = DIM_API_Copy.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : DIM_API_Copy");
        }
...
/************************************************************************************************
 *     141252 characters generated by Talend Open Studio for Data Integration 
