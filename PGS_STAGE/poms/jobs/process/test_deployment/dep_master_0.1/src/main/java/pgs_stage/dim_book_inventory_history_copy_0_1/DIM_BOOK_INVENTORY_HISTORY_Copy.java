...
package pgs_stage.dim_book_inventory_history_copy_0_1;
...
/**
 * Job: DIM_BOOK_INVENTORY_HISTORY_Copy Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class DIM_BOOK_INVENTORY_HISTORY_Copy implements TalendJob {
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	private final String jobName = "DIM_BOOK_INVENTORY_HISTORY_Copy";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				DIM_BOOK_INVENTORY_HISTORY_Copy.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(DIM_BOOK_INVENTORY_HISTORY_Copy.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
    final static byte[] commonByteArrayLock_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy = new byte[0];
    static byte[] commonByteArray_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy.length) {
				if(length < 1024 && commonByteArray_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy.length == 0) {
   					commonByteArray_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy = new byte[1024];
				} else {
   					commonByteArray_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy, 0, length);
			strReturn = new String(commonByteArray_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy, 0, length);
			strReturn = new String(commonByteArray_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_PGS_STAGE_DIM_BOOK_INVENTORY_HISTORY_Copy) {
...
String dbUser_tDBOutput_1 = null;
    ...
    String dbname_tDBOutput_1 = "deployment_test" ;
    String url_tDBOutput_1 = "jdbc:jtds:sqlserver://" + "LAPTOP-HED7JE2F" ; 
    if (!"".equals(port_tDBOutput_1)) {
    	url_tDBOutput_1 += ":" + "1433";
    }
    if (!"".equals(dbname_tDBOutput_1)) {
				url_tDBOutput_1 += "//" + "deployment_test"; 
	...
	final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:0gmTII8rKFSeA5eLhQHJNyQuIyzv9ZY2zOecK29gONhhG0KVD8k=");
...
if(dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
    tableName_tDBOutput_1 = "DIM_BOOK";
} else {
    tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "DIM_BOOK";
}
 ...
 */
	...
	final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:0t0ou8mRMFrRmh+GMEiKSAxhG3jMzyC+CKmakbG409JHRg4mY0k=");
...
} else {pstmt_tDBOutput_1.setString(2, row3.FOLIO_NUMBER);
}

                    if(row3.DATE != null) {
pstmt_tDBOutput_1.setTimestamp(3, new java.sql.Timestamp(row3.DATE.getTime()));
} else {
pstmt_tDBOutput_1.setNull(3, java.sql.Types.TIMESTAMP);
}

                    pstmt_tDBOutput_1.setInt(4, row3.BOOK_INVENTORY_ID);

                    if(row3.SI_SUPPLIER_NUMBER == null) {
pstmt_tDBOutput_1.setNull(5, java.sql.Types.INTEGER);
} else {pstmt_tDBOutput_1.setInt(5, row3.SI_SUPPLIER_NUMBER);
}

                    if(row3.SI_TERMINAL_PRODUCT_NUMBER == null) {
pstmt_tDBOutput_1.setNull(6, java.sql.Types.INTEGER);
} else {pstmt_tDBOutput_1.setInt(6, row3.SI_TERMINAL_PRODUCT_NUMBER);
}

                    pstmt_tDBOutput_1.setInt(7, row3.INVENTORY_QUANTITY_TYPE);

                    pstmt_tDBOutput_1.setInt(8, row3.OPENING_MTD_RECEIPTS);

                    pstmt_tDBOutput_1.setInt(9, row3.CLOSING_MTD_RECEIPTS);

                    pstmt_tDBOutput_1.setInt(10, row3.OPENING_MTD_FRAC_RECEIPTS);

                    pstmt_tDBOutput_1.setInt(11, row3.CLOSING_MTD_FRAC_RECEIPTS);

                    pstmt_tDBOutput_1.setInt(12, row3.OPENING_MTD_DISPOSALS);

                    pstmt_tDBOutput_1.setInt(13, row3.CLOSING_MTD_DISPOSALS);

                    pstmt_tDBOutput_1.setInt(14, row3.OPENING_MTD_FRAC_DISPOSALS);

                    pstmt_tDBOutput_1.setInt(15, row3.CLOSING_MTD_FRAC_DISPOSALS);

                    pstmt_tDBOutput_1.setInt(16, row3.FOLIO_RECEIPTS);

                    pstmt_tDBOutput_1.setInt(17, row3.FOLIO_FRAC_RECEIPTS);

                    pstmt_tDBOutput_1.setInt(18, row3.FOLIO_DISPOSALS);

                    pstmt_tDBOutput_1.setInt(19, row3.OPENING_INVENTORY);

                    pstmt_tDBOutput_1.setInt(20, row3.OPENING_FRAC_INVENTORY);

                    pstmt_tDBOutput_1.setInt(21, row3.CLOSING_INVENTORY);
 ...
 */
    ...
    public static void main(String[] args){
        final DIM_BOOK_INVENTORY_HISTORY_Copy DIM_BOOK_INVENTORY_HISTORY_CopyClass = new DIM_BOOK_INVENTORY_HISTORY_Copy();

        int exitCode = DIM_BOOK_INVENTORY_HISTORY_CopyClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = DIM_BOOK_INVENTORY_HISTORY_Copy.class.getClassLoader().getResourceAsStream("pgs_stage/dim_book_inventory_history_copy_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = DIM_BOOK_INVENTORY_HISTORY_Copy.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : DIM_BOOK_INVENTORY_HISTORY_Copy");
        }
...
/************************************************************************************************
 *     165077 characters generated by Talend Open Studio for Data Integration 
 *     on the 3 août 2024 à 14:40:32 GMT+01:00
 ************************************************************************************************/