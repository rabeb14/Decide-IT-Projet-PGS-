...
package pgs_stage.dim_gouvernerat_0_1;
...
/**
 * Job: Dim_gouvernerat Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class Dim_gouvernerat implements TalendJob {
	...
	private final String jobName = "Dim_gouvernerat";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Dim_gouvernerat.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Dim_gouvernerat.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_PGS_STAGE_Dim_gouvernerat = new byte[0];
		static byte[] commonByteArray_PGS_STAGE_Dim_gouvernerat = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PGS_STAGE_Dim_gouvernerat.length) {
					if (length < 1024 && commonByteArray_PGS_STAGE_Dim_gouvernerat.length == 0) {
						commonByteArray_PGS_STAGE_Dim_gouvernerat = new byte[1024];
					} else {
						commonByteArray_PGS_STAGE_Dim_gouvernerat = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PGS_STAGE_Dim_gouvernerat, 0, length);
				strReturn = new String(commonByteArray_PGS_STAGE_Dim_gouvernerat, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PGS_STAGE_Dim_gouvernerat, 0, length);
				strReturn = new String(commonByteArray_PGS_STAGE_Dim_gouvernerat, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PGS_STAGE_Dim_gouvernerat) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:6po95OCK0b7JQCZoV72/e0kLrCKjVpGWxJmkzBimYxwOwE8NGas=");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "DIM_GOUVERNERAT";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "DIM_GOUVERNERAT";
				}
				...
				try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
					stmtCreate_tDBOutput_1.execute("CREATE TABLE [" + tableName_tDBOutput_1
							+ "]([ADRESSE] VARCHAR(40)  ,[GOUVERNORAT] VARCHAR(21)  )");
				}
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([ADRESSE],[GOUVERNORAT]) VALUES (?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:tdSO5uLlxMihC+GQh+htqix9UgSwhHFN0Jrc8g==");
				...
				Object source_tFileInputExcel_1 = "C:/Users/user/Desktop/stage/gouvernorat.xlsx";
				...
				try {
					...
					if (sheetList_tFileInputExcel_1.size() > 0) {
						...
						int limit_tFileInputExcel_1 = 141;
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.ADRESSE == null) {
									...
									pstmt_tDBOutput_1.setString(1, row1.ADRESSE);
								}

								if (row1.GOUVERNORAT == null) {
									pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(2, row1.GOUVERNORAT);
								}
	...
	public static void main(String[] args) {
		final Dim_gouvernerat Dim_gouverneratClass = new Dim_gouvernerat();

		int exitCode = Dim_gouverneratClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Dim_gouvernerat.class.getClassLoader()
					.getResourceAsStream("pgs_stage/dim_gouvernerat_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Dim_gouvernerat.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : Dim_gouvernerat");
		}
...
/************************************************************************************************
 * 57583 characters generated by Talend Open Studio for Data Integration on the
 * 2 août 2024 à 12:21:53 GMT+01:00
 ************************************************************************************************/